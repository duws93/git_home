...
#coding:utf-8
#author: duws
#time: 2019-05-15 17:00

'''
usages: python shellname argv1 argv2
argv1: 生成zabbix哪个分组报表 example: dwstest
argv2: 需要统计流量的网卡名称 example: eth0 
'''
...
#生成文件名称：
xlsfilename = '/etc/zabbix/script/zabbix.xls'
in='net.if.out['+sys.argv[2]+']'
out='net.if.in['+sys.argv[2]+']'
...
keys = [
    ...
    ['进入最大流量（单位Kbps）','trends_uint',in,'max','',1000],
    ['进入平均流量（单位Kbps）','trends_uint',in,'avg','',1000],
    ['出去最大流量（单位Kbps）','trends_uint',out,'max','',1000],
    ['出去平均流量（单位Kbps）','trends_uint',out,'avg','',1000],
]
...
class ReportForm:
    ...
    def __init__(self):
        ...
        #生成zabbix哪个分组报表
        #self.groupname = 'dwstest'
        self.groupname = sys.argv[1]
    ...
    def writeToXls2(self):
        ...
        try:
            ...
            #创建工作薄
            worksheet = workbook.add_worksheet('zabbix')
